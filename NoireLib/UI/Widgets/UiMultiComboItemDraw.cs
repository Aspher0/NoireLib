...
/// <summary>
/// Everything one option of a <see cref="NoireMultiCombo{T}"/> knows about itself, handed to a custom renderer.
/// </summary>
...
/// <param name="Selected">Whether this option is currently ticked.</param>
/// <param name="Highlighted">Whether this option is the one the arrow keys are on.</param>
public readonly record struct UiMultiComboItemDraw<T>(
    NoireMultiCombo<T> Combo,
    T Item,
    int Index,
    string Display,
    bool Selected,
...
{
    /// <summary>
    /// Draws the option's tick box and steps past it, as the combo would draw it.
    /// </summary>
    public void DrawCheckbox() => Combo.DrawItemCheckbox(Selected);
