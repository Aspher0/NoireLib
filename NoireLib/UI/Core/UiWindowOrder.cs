...
/// front of that list; the next frame puts this one back, and the round trip shows up as a flicker. The layer keeps
/// the drawing immune to that churn, while the reordering keeps the input right.
/// </remarks>
internal static class UiWindowOrder
{
    ...
    /// It carries one more effect, to account for on a window with a background. ImGui reads a window's background
    /// colour from an index it picks by flag, and this flag selects <c>PopupBg</c> where an ordinary window would use
    /// <c>WindowBg</c>. A window that pushes <c>WindowBg</c> and then sets this flag is drawn in the theme's popup
    /// colour instead, silently and only once it is promoted.<br/>
    ...
    /// <c>End</c>: focusing any other window undoes it, so it is reapplied every frame rather than set once.
    /// </summary>
    /// <remarks>
    ...
    /// Within the top layer the order is still the display list, so the last window to call this each frame wins: a
    /// tooltip stays above an always-on-top element it overlaps because it is drawn after the thing it annotates, so
    /// it asks last.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// Read before the popup is opened, since inside one the current window is the popup itself. The layer lives on
    /// the root window, so this answers for a child region the same way it answers for the window around it.
    /// </remarks>
