...
public sealed partial class SceneNode
{
    ...
    /// <see cref="NoireInteract"/> if it isn't already running, and a non-interactable node is invisible to picking.
    /// </summary>
    ...
    /// <summary>
    /// Whether a left press on this node begins a drag (rather than only a click), default false; while true, a
    /// drag that starts on this node takes the mouse from the game the instant it is pressed, so the camera never
    /// pans underneath it, and it implies <see cref="Interactable"/> - set both.
    /// </summary>
    ...
    /// <summary>
    /// A node that stands in for this one when a click selects it; null, the default, selects the clicked node itself.<br/>
    /// This is how a model made of several meshes selects as one object: parent the mesh nodes under a group node
    /// and point each part's proxy at the group, so clicking any part selects (and gizmo-moves) the whole, while
    /// hover feedback, <see cref="OnClick"/> and the hit's triangle information stay on the part actually clicked -
    /// those answer "what is under the cursor", the proxy answers "what does picking it mean".<br/>
    ...
    /// <see cref="OnHoverExit"/> (never composed into them), and calling this again just replaces the transform,
    /// never stacking. Fluent.
    /// </summary>
    ...
    /// replaces the transform, never stacking. Fluent.
    /// </summary>
    /// <param name="hover">Tint transform applied while hovered; null uses <see cref="DefaultHoverHighlight"/> (x1.2), return the input unchanged for no visual change.</param>
