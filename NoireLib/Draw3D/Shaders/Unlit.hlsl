// Unlit shader. Variants: UNLIT_TEXTURED, UNLIT_INSTANCED, OPAQUE_DOMAIN.
#include "Common.hlsli"

struct VsIn
{
    float3 pos    : POSITION;
    float3 normal : NORMAL;      // unused, kept for one shared input layout
    float2 uv     : TEXCOORD0;
    float4 color  : COLOR0;
...
struct PsIn
{
    ...
    float2 uv     : TEXCOORD0;
    float4 color  : COLOR0;
    float2 clipZW : TEXCOORD1;   // device depth = z / w
};
...
#ifdef OPAQUE_DOMAIN
    // The Opaque bucket is unblended. DepthFade has no effect here.
    if (vis < 0.5) discard;
