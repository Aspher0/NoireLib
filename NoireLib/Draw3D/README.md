...
> A ground decal paints its shape onto the world surface. Actors you list with `ExcludeObjects(...)` are cut out of it, and with `NoireDraw3D.WorldOccludedDecals` on (default) the cut is driven by a **top-down collision height-map**: a listed actor's body is removed wherever it rises *above the local ground*, at any camera angle, while the ground (flat or sloped) and any furniture that has collision keep painting. An actor you *don't* list is painted over; world geometry is never falsely cut. The "ground" search is bounded by the **decal box's own Y scale** — a thin box only looks within its thickness, so a covering roof/ceiling is ignored unless the box is tall enough to reach it. Appendages that reach past the hitbox (a long tail) are covered by widening the exclusion radius — `ExcludeObjects(pred, radiusScale: 2f)` — which is safe here (a bigger cylinder can't cause a moat, since only *elevated* surfaces inside it are cut).

- `Projection = DecalProjection.HighestOnly` paints only the **topmost** surface *within the decal box* per column (the tabletop, not the floor beneath it), instead of every surface in the footprint (`AllSurfaces`, the default). "Topmost" is bounded by the box's Y scale, so scale the box to the vertical range you want to consider. It's purely vertical — it never affects wall/object occlusion. Needs `WorldOccludedDecals` and the covering object to have collision.
