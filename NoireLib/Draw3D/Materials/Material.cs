...
public sealed record Material
{
    ...
    /// <summary>
    /// How pixels blend into the Draw3D layer; <see cref="BlendMode.Opaque"/> also z-tests in hardware, and
    /// <see cref="MaterialDomain.GroundDecal"/> honours only <see cref="BlendMode.Additive"/> or <see cref="BlendMode.Premultiplied"/>
    /// (never opaque), via <see cref="Decal"/>'s <c>additive</c> flag.
    /// </summary>
    public BlendMode Blend { get; init; } = BlendMode.Premultiplied;

    /// <summary>
    /// Whether pixels are occluded by the game's world geometry; ignored by <see cref="MaterialDomain.GroundDecal"/>,
    /// where projection <i>replaces</i> testing.
    /// </summary>
    ...
    /// <summary>
    /// Optional second texture for custom pipelines (<c>AuxTex0</c>), ignored by the standard shaders and used by
    /// game materials for the normal map; referenced, never owned.
    /// </summary>
    public GpuTexture? AuxTexture0 { get; init; }

    /// <summary>
    /// Optional third texture for custom pipelines (<c>AuxTex1</c>), ignored by the standard shaders and used by
    /// game materials for the specular map; referenced, never owned.
    /// </summary>
    public GpuTexture? AuxTexture1 { get; init; }

    /// <summary>
    /// Soft-edge width in world units where the material meets world geometry (0 = hard edge); blended domains only,
    /// since <see cref="BlendMode.Opaque"/> occlusion is a hard pixel kill.
    /// </summary>
    ...
    /// <summary>
    /// <see cref="MaterialDomain.GroundDecal"/> only: outline band width against the unit footprint (SDF units, 0..1),
    /// held at a constant world thickness regardless of the decal's scale. 0 = no outline.
    /// </summary>
    public float OutlineWidth { get; init; }

    /// <summary>
    /// <see cref="MaterialDomain.GroundDecal"/> only: the outline rim's own color, straight alpha; alpha 0 (default)
    /// leaves the rim the decal's <see cref="Color"/>, an alpha above 0 colors the border independently of the fill.
    /// </summary>
    ...
    /// <summary>
    /// Extra per-material values for custom pipelines, arriving in the shader as <c>Params2</c>; ignored by the
    /// standard shaders, unusable by <see cref="MaterialDomain.GroundDecal"/> since its own shader needs that
    /// register for projection data.
    /// </summary>
    ...
    /// <summary>
    /// Opt-in: lets translucent draws with this material render out of back-to-front order for aggressive
    /// instancing; safe for many identical, non-overlapping markers, and always on for <see cref="BlendMode.Additive"/>
    /// (order-independent).
    /// </summary>
    ...
    /// <param name="outlineWidth">Outline band width (default 0.08); held at a constant world thickness regardless of the decal's scale, see <see cref="OutlineWidth"/>.</param>
    ...
    /// <param name="additive">True blends additively (stacked decals sum toward white); false is the standard translucent blend; equivalent to setting <see cref="Blend"/> to <see cref="BlendMode.Additive"/>.</param>
    /// <param name="outlineColor">The border's own color, straight alpha; null (default) matches the decal's <paramref name="color"/> so rim and fill differ only in opacity, pass one to color the border independently.</param>
    ...
    /// over the standard vertex layout; falls back to the <see cref="Domain"/> shader if the named pipeline isn't registered.
    /// </summary>
