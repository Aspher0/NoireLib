...
/// <summary>
/// Locks the registry capabilities that awaited, async dispatch depends on:
/// <see cref="NoireSubscriptionRegistry{TKey, TContext}.DispatchAsync"/> awaits async handlers while sharing the
/// filter/once/priority core with the synchronous dispatch, and the opt-in <c>propagateHandlerExceptions</c> mode
/// lets a handler fault surface to the dispatcher instead of being caught and reported.<br/>
/// The default catch-and-report behavior is covered by <see cref="NoireSubscriptionRegistryTests"/>; these tests
/// only exercise the two capabilities above.
/// </summary>
