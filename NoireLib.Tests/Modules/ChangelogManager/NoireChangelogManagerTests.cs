...
public class NoireChangelogManagerTests : IDisposable
{
    ...
    {
        // Versions are unpacked only after the window is constructed, so a non-null list must not throw here.
    ...
    {
        // An empty list goes through the same construction path and must not throw either.
